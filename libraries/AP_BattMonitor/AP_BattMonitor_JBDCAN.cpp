...
void AP_BattMonitor_JBDCAN::init() {
    ...
    if (!_iface->register_frame_callback(
        FUNCTOR_BIND_MEMBER(&AP_BattMonitor_JBDCAN::handle_frame_callback,
                            void, uint8_t, const AP_HAL::CANFrame &, AP_HAL::CANIface::CanIOFlags),
        _callback_id)) {
...
void AP_BattMonitor_JBDCAN::handle_frame_callback(uint8_t iface_num,
...
bool AP_BattMonitor_JBDCAN::capacity_remaining_pct(uint8_t &percentage) const {
    if (_params._pack_capacity > 0) {
        percentage = 100 * (_params._pack_capacity - _state.consumed_mah) / _params._pack_capacity;
    }
    return true;
}
