...
void SimRover::update_ackermann_or_skid(const struct sitl_input &input,
  ...
  // if in skid steering mode the steering and throttle values are used for
  // motor1 and motor2
  if (skid_steering) {
  ...
  // Default pitch simulation
  // --- LOGIC GIẢ LẬP DAO ĐỘNG PITCH (0 -> -7 -> 0) ---
  // Định nghĩa các trạng thái của quá trình giả lập
  enum class PitchSimState {
    IDLE,       // Chờ bắt đầu hoặc đã hoàn thành
    DECREASING, // Đang giảm từ 0 về -7
    INCREASING  // Đang tăng từ -7 về 0
  };

  static PitchSimState pitch_state = PitchSimState::DECREASING;
  static const float PITCH_LIMIT_DEG = -5.0f;
  // static const float PITCH_LIMIT_DEG = 0.0f; // Normal
  static const float PITCH_RESTORE_DEG = 0.0f;
  static const float PITCH_STEP_RATE = 0.001f; // rad/s (Tốc độ thay đổi góc)

  float sim_roll, sim_pitch, sim_yaw;
  dcm.to_euler(&sim_roll, &sim_pitch, &sim_yaw);
  float current_pitch_deg = degrees(sim_pitch);

  float pitch_gyro_output = 0.0f;

  switch (pitch_state) {
  case PitchSimState::DECREASING:
    // Pha 1: Giảm dần xuống  độ
    if (current_pitch_deg > PITCH_LIMIT_DEG) {
      pitch_gyro_output = -PITCH_STEP_RATE;
    } else {
      // Đã chạm ngưỡng , chuyển trạng thái tăng
      pitch_state = PitchSimState::INCREASING;
    }
    break;

  case PitchSimState::INCREASING:
    // Pha 2: Tăng dần lại về 0 độ
    if (current_pitch_deg < PITCH_RESTORE_DEG) {
      pitch_gyro_output = PITCH_STEP_RATE;
    } else {
      // Đã về 0, dừng giả lập (IDLE)
      pitch_state = PitchSimState::IDLE;
    }
    break;

  case PitchSimState::IDLE:
  default:
    pitch_gyro_output = 0.0f;
    break;
  }

  // Cập nhật gyro trục Y (Pitch) và giữ nguyên Yaw từ tính toán lái
  gyro = Vector3f(0.0f, pitch_gyro_output, radians(yaw_rate));
  // --------------------------------------------------
  // update attitude
