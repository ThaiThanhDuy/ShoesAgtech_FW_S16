...
const AP_Param::GroupInfo AP_ShoesAgtech::var_info[] = {
    ...
    // @Param: CAL_FAC
    // @DisplayName: Flow sensor calibration (pulses/Litre)
    // @User: Standard
    ...
    // @Description: Print spray mode, flow target/actual/avg and pump PWM every 1s.
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    ...
    //   giving identical behaviour to RC passthrough without blocking modes 1/2.
    // @Range: 1 16
    // @User: Standard
    ...
    // [AP_ShoesAgtech] pH sensor parameters — Nengshi ASPS3801D-0.5M (slots 14-19)

    // @Param: PH_EN
    // @DisplayName: Enable pH sensor
    // @Description: Enable Nengshi ASPS3801D-0.5M pH sensor via Modbus RTU (RS485-TTL)
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    ...
    // @Description: Set to match the SERIALx port connected to the RS485-TTL module.
    ...
    // @Description: Added to the raw sensor temperature reading after /10 decode.
    // @Range: -10 10
    // @User: Standard
    ...
    // @Description: Added to decoded pH value. Use buffer solution to determine.
    // @Range: -2.0 2.0
    // @User: Standard
    ...
    // @Description: Reference alkalinity measured by test kit (dKH). Used as base
    ...
    // @Description: Exponential Moving Average coefficient. Lower = smoother but slower.
    // @Range: 0.01 1.0
    // @User: Advanced
    ...
    // @Description: SA_LOG_EN controls flow/spray console log. SA_PH_LOG controls
    ...
    // @Description: Local time = UTC + PH_TZ. Vietnam is UTC+7 (default). Used to
    //   classify pH readings into morning (05:00-11:59) and afternoon (12:00-16:59)
...
AP_ShoesAgtech::AP_ShoesAgtech()
    : _last_timestamp_ms(0), _last_pulse_snapshot(0), _last_log_ms(0),
      ...
      _buffer_index(0), _buffer_sum(0.0f), _samples_count(0),
      _spray_mode(0), _pump_pwm(0), _flow_target(0.0f),
      _pid_integral(0.0f), _pid_output_lpf(0.0f), _pid_last_ms(0),
      _last_pump_chan(-1), _last_pump_func_val(-1),
      _pump_config_ok(false), _last_warn_ms(0),
      // [AP_ShoesAgtech] pH sensor initial state
      _ph_uart(nullptr),
      _ph_update_ms(0), _ph_req_sent_ms(0), _ph_req_pending(false),
      _ph_value(0.0f), _ph_value_ema(-1.0f), _ph_value_ma(0.0f),
      _ph_mv(0), _ph_temp(25.0f),
      _alk_dkh(0.0f), _alk_mgl(0.0f),
      _ph_buf_idx(0), _ph_buf_count(0), _ph_buf_sum(0.0f),
      // daily slot tracking
      _ph_morn_val(0.0f), _ph_aft_val(0.0f),
      _ph_morn_valid(false), _ph_aft_valid(false),
      _delta_ph(0.0f),
      _alk_today_dkh(0.0f), _alk_today_mgl(0.0f),
      _alk_prev_dkh(0.0f), _alk_prev_mgl(0.0f),
      _alk_slot_status(4),
...
void AP_ShoesAgtech::init(void) {
  ...
  if (!hal.gpio->attach_interrupt(55, irq_handler, AP_HAL::GPIO::INTERRUPT_RISING)) {
...
void AP_ShoesAgtech::update(void) {
  ...
  if (delta_t_ms >= 100) {
    ...
    if (_samples_count < WINDOW_SIZE) { _samples_count++; }
    if (_samples_count > 0) { _flow_rate_avg = _buffer_sum / _samples_count; }
  }

  // ---- 2. SPRAY CONTROL ----
  float dt_pid = (now - _pid_last_ms) * 0.001f;
  if (dt_pid <= 0.0f || dt_pid > 1.0f) { dt_pid = 0.1f; }
  _pid_last_ms = now;
  ...
  case 0: {
    ...
    if (rc_pwm < 800 || rc_pwm > 2200) { rc_pwm = 1500; }
    _flow_target    = 0.0f;
    _pid_integral   = 0.0f;
    _pid_output_lpf = 0.0f;
    _pump_pwm = rc_pwm;
  ...
  if (_flow_log_enable.get() > 0 && now - _last_log_ms >= 1000) {
    _last_log_ms = now;
    gcs().send_text(MAV_SEVERITY_INFO,
                    "[FLOW] M%u Tgt:%.1f Act:%.1f Avg:%.1f PWM:%u",
                    (unsigned)_spray_mode,
                    (double)_flow_target,
                    (double)_flow_rate_filtered,
                    (double)_flow_rate_avg,
                    (unsigned)_pump_pwm);
  }
}
  ...
  if (func_val != (int32_t)SRV_Channel::k_none) {
    ...
    if (changed || now - _last_warn_ms >= 5000) {
      ...
      gcs().send_text(MAV_SEVERITY_WARNING,
                      "SA: SERVO%d_FUNCTION=%d must be 0(None)!",
                      (int)chan, (int)func_val);
    }
    ...
    if (ch != nullptr) {
      gcs().send_text(MAV_SEVERITY_INFO,
                      "SA: SERVO%d OK Min:%u Trim:%u Max:%u",
                      (int)chan,
                      (unsigned)ch->get_output_min(),
                      (unsigned)ch->get_trim(),
                      (unsigned)ch->get_output_max());
    }
...
uint16_t AP_ShoesAgtech::_run_flow_pid(float target_lmin, float dt) {
  ...
  return (uint16_t)constrain_float(
      (float)pwm_trim + _pid_output_lpf,
      (float)pwm_min,
      (float)pwm_max);
}
...
void AP_ShoesAgtech::_ph_init(void) {
  ...
  if (_ph_uart == nullptr) {
    gcs().send_text(MAV_SEVERITY_WARNING,
                    "SA: pH sensor SERIAL%d not found", (int)port_num);
    return;
  }
...
void AP_ShoesAgtech::_ph_update(void) {
  ...
  if (!_ph_req_pending) {
    ...
    while (stale > 0) { _ph_uart->read(); stale--; }

    // Modbus FC04: read 9 registers from 0x0000, CRC = 0x0C30 (LSB first: 30 0C)
    static const uint8_t req[8] = {0x01, 0x04, 0x00, 0x00, 0x00, 0x09, 0x30, 0x0C};
  ...
  // Timeout: if no full frame within 500ms, give up
  if (avail < 23) {
  ...
  if (_ph_buf_count < PH_WINDOW) { _ph_buf_count++; }
  ...
  if (_ph_log_enable.get() > 0) {
    ...
    switch (_alk_slot_status) {
      case 0:  slot_tag = "FULL";     break;  // both morning + afternoon today
      case 1:  slot_tag = "MORN";     break;  // morning only
      case 2:  slot_tag = "AFT";      break;  // afternoon only
      case 3:  slot_tag = "PREV";     break;  // yesterday's data
      default: slot_tag = "NODATA";   break;  // no data yet
    }
    gcs().send_text(MAV_SEVERITY_INFO,
                    "[WM] pH:%.2f MA:%.2f Tmp:%.1fC mV:%d",
                    (double)_ph_value,
                    (double)_ph_value_ma,
                    (double)_ph_temp,
                    (int)_ph_mv);
    gcs().send_text(MAV_SEVERITY_INFO,
                    "[WM] Alk:%.2fdKH %.1fmg/L dPH:%+.2f [%s]",
                    (double)_alk_dkh,
                    (double)_alk_mgl,
                    (double)_delta_ph,
                    slot_tag);
...
void AP_ShoesAgtech::_ph_update_daily_slots(float ph_cal) {
  ...
  // ---- GPS time classification (optional — slot tracking only works with GPS time) ----
  uint64_t utc_usec = 0;
  ...
  if (_ph_morn_valid && _ph_aft_valid) {
    ...
    float kh_scaled = _ph_kh.get() * (1.0f + constrain_float(_delta_ph * 0.375f, -0.5f, 1.0f));
    ...
    // No slotted data at all: single-point heuristic from current pH so display is not 0
...
// =============================================================
float AP_ShoesAgtech::_ph_calc_alkalinity(float ph, float base_kh_dkh, float temp_c) {
