...
#define AC_PID_RESET_TC                                                        \
  0.16f // Time constant for integrator reset decay to zero
...
public:
  ...
  AC_PID(float initial_p, float initial_i, float initial_d, float initial_ff,
         float initial_imax, float initial_filt_T_hz, float initial_filt_E_hz,
         float initial_filt_D_hz, float initial_srmax = 0,
         float initial_srtau = 1.0, float initial_dff = 0);
  AC_PID(const AC_PID::Defaults &defaults)
      : AC_PID(defaults.p, defaults.i, defaults.d, defaults.ff, defaults.imax,
               defaults.filt_T_hz, defaults.filt_E_hz, defaults.filt_D_hz,
               defaults.srmax, defaults.srtau, defaults.dff) {}
  ...
  float update_all(float target, float measurement, float dt,
                   bool limit = false, float pd_scale = 1.0f);

  // Computes the PID output from an error input only (target assumed to be
  // zero). Applies error filtering and updates the derivative and integrator.
  ...
  // Flags the input filter for reset. The next call to `update_all()` will
  // reinitialize the filter using the next input.
  ...
  // Returns alpha value for the target low-pass filter (based on filter
  // frequency and dt)
  float get_filt_T_alpha(float dt) const;
  // Returns alpha value for the error low-pass filter (based on filter
  // frequency and dt)
  ...
  // Gradually adjust the integrator toward a desired value using a time
  // constant. Typically used to "relax" the I-term in dynamic conditions.
  ...
  // Returns current slew rate from the limiter. Returns 0 if SMAX is zero
  // (disabled).
...
protected:
  ...
  // Slew rate time constant (tau). Not exposed in this class by default, but
  // defined as an AP_Float so parent classes can make it configurable via param
  // table.
  ...
  // flags
  struct ac_pid_flags {
    bool _reset_filter : 1; // true if the input filter should be reset on the
                            // next call to update_all()
    bool _I_set : 1; // true if the I term has been set externally, including
                     // zeroing
  } _flags;
