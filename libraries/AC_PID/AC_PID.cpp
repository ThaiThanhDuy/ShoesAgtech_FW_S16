/// @file	AC_PID.cpp
/// @brief	General-purpose PID controller with input, error, and derivative
/// filtering, plus slew rate limiting and EEPROM gain storage.
...
const AP_Param::GroupInfo AC_PID::var_info[] = {
    ...
    // @Description: P Gain which produces an output value that is proportional
    // to the current error value
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("P", 0, AC_PID, _kp, default_kp),

    // @Param: I
    // @DisplayName: PID Integral Gain
    // @Description: I Gain which produces an output that is proportional to
    // both the magnitude and the duration of the error
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("I", 1, AC_PID, _ki, default_ki),

    // @Param: D
    // @DisplayName: PID Derivative Gain
    // @Description: D Gain which produces an output that is proportional to the
    ...
    // @Description: FF Gain which produces an output value that is proportional
    // to the demanded input
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FF", 4, AC_PID, _kff, default_kff),

    // @Param: IMAX
    // @DisplayName: PID Integral Maximum
    // @Description: The maximum/minimum value that the I term can output
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("IMAX", 5, AC_PID, _kimax,
                                       default_kimax),
    ...
    // @Units: Hz
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FLTT", 9, AC_PID, _filt_T_hz,
                                       default_filt_T_hz),
    ...
    // @Units: Hz
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FLTE", 10, AC_PID, _filt_E_hz,
                                       default_filt_E_hz),
    ...
    // @Units: Hz
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("FLTD", 11, AC_PID, _filt_D_hz,
                                       default_filt_D_hz),
    ...
    // @Increment: 0.5
    // @User: Advanced
    AP_GROUPINFO_FLAGS_DEFAULT_POINTER("SMAX", 12, AC_PID, _slew_rate_max,
                                       default_slew_rate_max),

    // @Param: PDMX
    // @DisplayName: PD sum maximum
    // @Description: The maximum/minimum value that the sum of the P and D term
    // can output
    // @User: Advanced
...
// Constructor
AC_PID::AC_PID(float initial_p, float initial_i, float initial_d,
               float initial_ff, float initial_imax, float initial_filt_T_hz,
               float initial_filt_E_hz, float initial_filt_D_hz,
               float initial_srmax, float initial_srtau, float initial_dff)
    : default_kp(initial_p), default_ki(initial_i), default_kd(initial_d),
      default_kff(initial_ff), default_kdff(initial_dff),
      default_kimax(initial_imax), default_filt_T_hz(initial_filt_T_hz),
  ...
  // this param is not in the table, so its default is no loaded in the call
  // above
...
void AC_PID::set_filt_T_hz(float hz) { _filt_T_hz.set(fabsf(hz)); }

// Sets the error low-pass filter frequency (Hz)
void AC_PID::set_filt_E_hz(float hz) { _filt_E_hz.set(fabsf(hz)); }

// Sets the derivative low-pass filter frequency (Hz)
void AC_PID::set_filt_D_hz(float hz) { _filt_D_hz.set(fabsf(hz)); }

// slew_limit - set slew limit
void AC_PID::set_slew_limit(float smax) { _slew_rate_max.set(fabsf(smax)); }

// Configures optional notch filters for target and error signals using the
// given sample rate. Filters are dynamically allocated and validated via the
// AP_Filter API.
void AC_PID::set_notch_sample_rate(float sample_rate) {
  ...
  if (_notch_E_filter != 0) {
    ...
    if (filter != nullptr &&
        !filter->setup_notch_filter(*_error_notch, sample_rate)) {
      delete _error_notch;
...
// Applies filters to the target and error, calculates the derivative and
// updates the integrator. If `limit` is true, the integrator is allowed to
// shrink but not grow.
...
float AC_PID::update_error(float error, float dt, bool limit) {
  ...
  // Reuse update all code path, zero target and pass negative error as
  // measurement Pass negative error as "measurement" so that error = target -
  // measurement evaluates correctly Bypasses target filtering for legacy
  // compatibility
  _target = 0.0;
...
// If `limit` is true, the integrator is only allowed to shrink to avoid
// wind-up.
void AC_PID::update_i(float dt, bool limit) {
  if (!is_zero(_ki) && is_positive(dt)) {
    // Allow integrator growth only if not limited, or if error opposes the
...
float AC_PID::get_p() const { return _pid_info.P; }

float AC_PID::get_i() const { return _integrator; }

float AC_PID::get_d() const { return _pid_info.D; }

float AC_PID::get_ff() const { return _pid_info.FF + _pid_info.DFF; }
...
// Loads controller configuration from EEPROM, including gains and filter
...
// Saves controller configuration from EEPROM, including gains and filter
// frequencies. Used by autotune to save gains before tuning.
...
// Returns alpha value for the target low-pass filter (based on filter frequency
// and dt)
float AC_PID::get_filt_T_alpha(float dt) const {
  return calc_lowpass_alpha_dt(dt, _filt_T_hz);
}

// Returns alpha value for the error low-pass filter (based on filter frequency
// and dt)
float AC_PID::get_filt_E_alpha(float dt) const {
  return calc_lowpass_alpha_dt(dt, _filt_E_hz);
}

// Returns alpha value for the derivative low-pass filter (based on filter
// frequency and dt)
float AC_PID::get_filt_D_alpha(float dt) const {
  return calc_lowpass_alpha_dt(dt, _filt_D_hz);
}

// Sets the integrator directly, clamped to the IMAX bounds. Also flags I-term
// as externally set.
