...
static void failsafe_check_static() { rover.failsafe_check(); }
...
void Rover::init_ardupilot() {
  ...
  g2.motors.init(get_frame_type()); // init motors including setting servo out
                                    // channels ranges
  ...
  Mode *initial_mode =
      mode_from_mode_num((enum Mode::Number)g.initial_mode.get());
  ...
  rc().convert_options(RC_Channel::AUX_FUNC::ARMDISARM_UNUSED,
                       RC_Channel::AUX_FUNC::ARMDISARM);
  rc().convert_options(RC_Channel::AUX_FUNC::SAVE_TRIM,
                       RC_Channel::AUX_FUNC::TRIM_TO_CURRENT_SERVO_RC);
  rc().init();
  ...
  if (is_boat()) {
    rover.g2.mis_done_behave.set_default(
        uint8_t(ModeAuto::DoneBehaviour::LOITER));
  }

  // flag that initialisation has completed
  initialised = true;

  // [AP_ShoesAgtech] attach GPIO IRQ for flow sensor, pump channel check
  g2.custom_nav.init();
  // [/AP_ShoesAgtech]
}
...
void Rover::update_ahrs_flyforward() {
  ...
  if (!is_boat()) {
    ...
    bool throttle_over_thresh =
        g2.motors.get_throttle() > MIN(g.throttle_cruise * 0.50f, 15.0f);
    // desired speed threshold of 1m/s
    bool desired_speed_over_thresh =
        g2.attitude_control.speed_control_active() &&
        (g2.attitude_control.get_desired_speed() > 0.5f);
    if (throttle_over_thresh ||
        (is_positive(g2.motors.get_throttle()) && desired_speed_over_thresh)) {
...
bool Rover::gcs_mode_enabled(const Mode::Number mode_num) const {
  ...
  static const uint8_t mode_list[]{
      (uint8_t)Mode::Number::MANUAL,   (uint8_t)Mode::Number::ACRO,
      (uint8_t)Mode::Number::STEERING, (uint8_t)Mode::Number::LOITER,
      (uint8_t)Mode::Number::FOLLOW,   (uint8_t)Mode::Number::SIMPLE,
      (uint8_t)Mode::Number::CIRCLE,   (uint8_t)Mode::Number::AUTO,
      (uint8_t)Mode::Number::RTL,      (uint8_t)Mode::Number::SMART_RTL,
  ...
  return !block_GCS_mode_change((uint8_t)mode_num, mode_list,
...
bool Rover::set_mode(Mode &new_mode, ModeReason reason) {
  ...
  if ((reason == ModeReason::GCS_COMMAND) &&
      !gcs_mode_enabled((Mode::Number)new_mode.mode_number())) {
    GCS_SEND_TEXT(
        MAV_SEVERITY_NOTICE,
        "Mode change to %s denied, GCS entry disabled (FLTMODE_GCSBLOCK)",
        new_mode.name4());
    return false;
  }
...
#if AP_FENCE_ENABLED
  // pilot requested flight mode change during a fence breach indicates pilot is
  // attempting to manually recover this flight mode change could be automatic
  // (i.e. fence, battery, GPS or GCS failsafe) but it should be harmless to
  // disable the fence temporarily in these situations as well
...
bool Rover::set_mode(const uint8_t new_mode, ModeReason reason) {
  static_assert(sizeof(Mode::Number) == sizeof(new_mode),
 ...
 */
bool Rover::should_log(uint32_t mask) { return logger.should_log(mask); }
#endif

// returns true if vehicle is a boat
// this affects whether the vehicle tries to maintain position after reaching
// waypoints
