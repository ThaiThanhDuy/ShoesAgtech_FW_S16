...
uint8_t GCS_MAVLINK_Rover::base_mode() const {
  ...
  if (rover.g2.stick_mixing > 0 &&
      rover.control_mode != &rover.mode_initializing) {
...
void GCS_MAVLINK_Rover::send_nav_controller_output() const {
  ...
  mavlink_msg_nav_controller_output_send(
      ...
      control_mode->nav_bearing(), control_mode->wp_bearing(),
      MIN(control_mode->get_distance_to_destination(), UINT16_MAX), 0,
      control_mode->speed_error(), control_mode->crosstrack_error());
}
...
void GCS_MAVLINK_Rover::send_servo_out() {
  ...
  if (rover.g2.motors.have_skid_steering()) {
    motor1 =
        10000 *
        (SRV_Channels::get_output_scaled(SRV_Channel::k_throttleLeft) * 0.001f);
    motor3 =
        10000 * (SRV_Channels::get_output_scaled(SRV_Channel::k_throttleRight) *
                 0.001f);
  } else {
    motor1 = 10000 * (SRV_Channels::get_output_scaled(SRV_Channel::k_steering) /
                      4500.0f);
    motor3 = 10000 *
             (SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) * 0.01f);
  }
  mavlink_msg_rc_channels_scaled_send(chan, millis(),
                                      0, // port 0
                                      motor1, 0, motor3, 0, 0, 0, 0, 0,
#if AP_RSSI_ENABLED
                                      receiver_rssi()
#else
                                      255
...
void GCS_MAVLINK_Rover::send_rangefinder() const {
  ...
  for (uint8_t i = 0; i < rover.rangefinder.num_sensors(); i++) {
    ...
    if (!got_one || s->distance() < distance) {
      distance = s->distance();
  ...
  mavlink_msg_rangefinder_send(chan, distance, voltage);
}
...
void GCS_MAVLINK_Rover::send_water_depth() {
  ...
  for (uint8_t i = 0; i < num_sensors; i++) {
    ...
    const AP_RangeFinder_Backend *s =
        rangefinder->get_backend(last_WATER_DEPTH_index);

    if (s == nullptr || s->orientation() != ROTATION_PITCH_270 ||
        !s->has_data()) {
      continue;
    }
    ...
    mavlink_msg_water_depth_send(
        chan,
        AP_HAL::millis(),       // time since system boot TODO: take time of
                                // measurement
...
void GCS_MAVLINK_Rover::send_pid_tuning() {
  ...
  if (g.gcs_pid_mask & 1) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, PID_TUNING_STEER, degrees(pid_info->target),
        degrees(pid_info->actual), pid_info->FF, pid_info->P, pid_info->I,
        pid_info->D, pid_info->slew_rate, pid_info->Dmod);
  ...
  if (g.gcs_pid_mask & 2) {
    ...
    mavlink_msg_pid_tuning_send(chan, PID_TUNING_ACCZ, pid_info->target,
                                pid_info->actual, pid_info->FF, pid_info->P,
                                pid_info->I, pid_info->D, pid_info->slew_rate,
                                pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
    }
  ...
  if (g.gcs_pid_mask & 4) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, PID_TUNING_PITCH, degrees(pid_info->target),
        degrees(pid_info->actual), pid_info->FF, pid_info->P, pid_info->I,
        pid_info->D, pid_info->slew_rate, pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
  ...
  if (g.gcs_pid_mask & 8) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, 7, pid_info->target, pid_info->actual, pid_info->FF, pid_info->P,
        pid_info->I, pid_info->D, pid_info->slew_rate, pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
    }
  }
  ...
  if (g.gcs_pid_mask & 16) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, 8, pid_info->target, pid_info->actual, pid_info->FF, pid_info->P,
        pid_info->I, pid_info->D, pid_info->slew_rate, pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
    }
  ...
  if (g.gcs_pid_mask & 32) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, 9, pid_info->target, pid_info->actual, pid_info->FF, pid_info->P,
        pid_info->I, pid_info->D, pid_info->slew_rate, pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
    }
  ...
  if (g.gcs_pid_mask & 64) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, 10, pid_info->target, pid_info->actual, pid_info->FF, pid_info->P,
        pid_info->I, pid_info->D, pid_info->slew_rate, pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
    }
  ...
  if (g.gcs_pid_mask & 128) {
    ...
    mavlink_msg_pid_tuning_send(
        chan, 11, pid_info->target, pid_info->actual, pid_info->FF, pid_info->P,
        pid_info->I, pid_info->D, pid_info->slew_rate, pid_info->Dmod);
    if (!HAVE_PAYLOAD_SPACE(chan, PID_TUNING)) {
      return;
    }
...
void Rover::send_wheel_encoder_distance(const mavlink_channel_t chan) {
  ...
  if (g2.wheel_encoder.num_sensors() > 0) {
    ...
    mavlink_msg_wheel_distance_send(chan, 1000UL * AP_HAL::millis(),
                                    g2.wheel_encoder.num_sensors(), distances);
  }
}
...
bool GCS_MAVLINK_Rover::handle_guided_request(
...
MAV_RESULT GCS_MAVLINK_Rover::_handle_command_preflight_calibration(
...
MAV_RESULT GCS_MAVLINK_Rover::handle_command_int_packet(
  ...
  case MAV_CMD_DO_CHANGE_SPEED:
    ...
    case SPEED_TYPE_AIRSPEED: // Airspeed is treated as ground speed for GCS
                              // compatibility
  ...
  case MAV_CMD_DO_MOTOR_TEST:
    // param1 : motor sequence number (a number from 1 to max number of motors
    // on the vehicle) param2 : throttle type (0=throttle percentage, 1=PWM,
    // 2=pilot throttle channel pass-through. See MOTOR_TEST_THROTTLE_TYPE enum)
    // param3 : throttle (range depends upon param2)
    // param4 : timeout (in seconds)
    return rover.mavlink_motor_test_start(
        *this, (AP_MotorsUGV::motor_test_order)packet.param1,
        static_cast<uint8_t>(packet.param2),
        static_cast<int16_t>(packet.param3), packet.param4);
...
#if AP_MAVLINK_MAV_CMD_NAV_SET_YAW_SPEED_ENABLED
MAV_RESULT GCS_MAVLINK_Rover::handle_command_nav_set_yaw_speed(
    const mavlink_command_int_t &packet, const mavlink_message_t &msg) {
  ...
  if (packet.param3 > 0) {
    // relative angle
    rover.mode_guided.set_desired_heading_delta_and_speed(
        packet.param1 * 100.0f, packet.param2);
  } else {
    // absolute angle
    rover.mode_guided.set_desired_heading_and_speed(packet.param1 * 100.0f,
                                                    packet.param2);
  }
  return MAV_RESULT_ACCEPTED;
}
...
MAV_RESULT GCS_MAVLINK_Rover::handle_command_int_do_reposition(
    const mavlink_command_int_t &packet) {
  const bool change_modes =
      ((int32_t)packet.param2 & MAV_DO_REPOSITION_FLAGS_CHANGE_MODE) ==
...
void GCS_MAVLINK_Rover::handle_manual_control_axes(
    const mavlink_manual_control_t &packet, const uint32_t tnow) {
...
void GCS_MAVLINK_Rover::handle_set_attitude_target(
  ...
  const float target_speed =
      rover.control_mode->get_speed_default() * packet.thrust;
  ...
  if ((packet.type_mask & MAVLINK_SET_ATT_TYPE_MASK_YAW_RATE_IGNORE) != 0) {
    ...
    float target_heading_cd =
        degrees(Quaternion(packet.q[0], packet.q[1], packet.q[2], packet.q[3])
                    .get_euler_yaw()) *
        100.0f;
    rover.mode_guided.set_desired_heading_and_speed(target_heading_cd,
                                                    target_speed);
  } else {
    // use body_yaw_rate field
    rover.mode_guided.set_desired_turn_rate_and_speed(
...
// informing them:
void GCS_MAVLINK_Rover::send_acc_ignore_must_be_set_message(
    const char *msgname) {
  GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ignoring %s; set ACC_IGNORE in mask",
                msgname);
}
...
void GCS_MAVLINK_Rover::handle_set_position_target_local_ned(
  ...
  bool yaw_rate_ignore =
      packet.type_mask & MAVLINK_SET_POS_TYPE_MASK_YAW_RATE_IGNORE;
  ...
  if (!pos_ignore) {
    ...
    case MAV_FRAME_BODY_OFFSET_NED: {
      ...
      const float ne_x =
          packet.x * rover.ahrs.cos_yaw() - packet.y * rover.ahrs.sin_yaw();
      const float ne_y =
          packet.x * rover.ahrs.sin_yaw() + packet.y * rover.ahrs.cos_yaw();
  ...
  if (!vel_ignore) {
    ...
    target_speed = constrain_float(safe_sqrt(sq(packet.vx) + sq(packet.vy)),
                                   -speed_max, speed_max);
  ...
  if (!vel_ignore && (!yaw_ignore || !yaw_rate_ignore)) {
    ...
    // the frame may have been provided in MAV_FRAME_LOCAL_OFFSET_NED or
    // MAV_FRAME_LOCAL_NED
  ...
  if (!vel_ignore && yaw_ignore && yaw_rate_ignore) {
    // consume velocity
    rover.mode_guided.set_desired_heading_and_speed(target_yaw_cd,
                                                    speed_dir * target_speed);
  } else if (!vel_ignore && yaw_ignore && !yaw_rate_ignore) {
    // consume velocity and turn rate
    rover.mode_guided.set_desired_turn_rate_and_speed(target_turn_rate_cds,
                                                      speed_dir * target_speed);
  } else if (!vel_ignore && !yaw_ignore && yaw_rate_ignore) {
    ...
    // consume just target heading (probably only skid steering vehicles can do
    // this)
...
*/
void GCS_MAVLINK_Rover::handle_landing_target(
    const mavlink_landing_target_t &packet, uint32_t timestamp_ms) {
...
uint8_t GCS_MAVLINK_Rover::high_latency_tgt_airspeed() const {
  ...
  if (rover.control_mode->is_autopilot_mode()) {
    ...
    return MIN((vfr_hud_airspeed() - control_mode->speed_error()) * 5,
               UINT8_MAX);
  }
  return 0;
}
...
uint8_t GCS_MAVLINK_Rover::high_latency_wind_direction() const {
  if (rover.g2.windvane.enabled()) {
    ...
    return wrap_360(degrees(rover.g2.windvane.get_true_wind_direction_rad())) /
           2;
  }
  return 0;
}
...
// Send the mode with the given index (not mode number!) return the total number
// of modes Index starts at 1
uint8_t GCS_MAVLINK_Rover::send_available_mode(uint8_t index) const {
  ...
  mavlink_msg_available_modes_send(
      chan, mode_count, index,
      MAV_STANDARD_MODE::MAV_STANDARD_MODE_NON_STANDARD, mode_number,
