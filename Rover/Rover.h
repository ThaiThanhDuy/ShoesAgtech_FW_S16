...
// [AP_ShoesAgtech] include needed so Rover can access g2.custom_nav methods
...
private:
  ...
  struct {
    uint8_t bits;        // bit flags of failsafes that have started (but not
                         // necessarily triggered an action)
  ...
  // Ground speed
  // The amount current ground speed is below min ground speed.  meters per
  // second
  float ground_speed;

  // Battery Sensors
  AP_BattMonitor battery{MASK_LOG_CURRENT,
                         FUNCTOR_BIND_MEMBER(&Rover::handle_battery_failsafe,
  ...
  float wheel_encoder_last_distance_m
      [WHEELENCODER_MAX_INSTANCES]; // total distance recorded by wheel encoder
                                    // (for reporting to GCS)
  bool wheel_encoder_initialised;   // true once arrays below have been
                                    // initialised to sensors initial values
  float wheel_encoder_last_angle_rad
      [WHEELENCODER_MAX_INSTANCES]; // distance in radians at time of last
                                    // update to EKF
  uint32_t wheel_encoder_last_reading_ms
      [WHEELENCODER_MAX_INSTANCES];      // system time of last ping from each
                                         // encoder
  uint8_t wheel_encoder_last_index_sent; // index of the last wheel encoder sent
                                         // to the EKF
  ...
  bool get_control_output(AP_Vehicle::ControlOutput control_output,
                          float &control_value) override;
  bool nav_scripting_enable(uint8_t mode) override;
  bool nav_script_time(uint16_t &id, uint8_t &cmd, float &arg1, float &arg2,
                       int16_t &arg3, int16_t &arg4) override;
  ...
  void get_scheduler_tasks(const AP_Scheduler::Task *&tasks,
                           uint8_t &task_count, uint32_t &log_bit) override;
  ...
  // [AP_ShoesAgtech] 10Hz scheduler task: update sensor + log + spray control
...
public:
  ...
  bool mavlink_motor_test_check(const GCS_MAVLINK &gcs_chan, bool check_rc,
                                AP_MotorsUGV::motor_test_order motor_instance,
                                uint8_t throttle_type, int16_t throttle_value);
  MAV_RESULT
