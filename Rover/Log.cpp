...
void Rover::Log_Write_Attitude() {
  ...
  logger.Write_PID(LOG_PIDS_MSG,
                   g2.attitude_control.get_steering_rate_pid().get_pid_info());
  logger.Write_PID(LOG_PIDA_MSG,
                   g2.attitude_control.get_throttle_speed_pid_info());

  // log pitch control for balance bots
  if (is_balancebot()) {
    logger.Write_PID(
        LOG_PIDP_MSG,
        g2.attitude_control.get_pitch_to_throttle_pid().get_pid_info());
  }
  ...
  if (g2.sailboat.sail_enabled()) {
    logger.Write_PID(
        LOG_PIDR_MSG,
        g2.attitude_control.get_sailboat_heel_pid().get_pid_info());
  }
}
...
void Rover::Log_Write_Depth() {
  ...
  for (uint8_t i = 0; i < rangefinder.num_sensors(); i++) {
    ...
    logger.Write("DPTH", "TimeUS,Inst,Lat,Lng,Depth,Temp", "s#DUmO", "F-GG00",
                 "QBLLff", AP_HAL::micros64(), i, loc.lat, loc.lng,
                 (double)(s->distance()), temp_C);
  }
}
#endif
...
void Rover::Log_Write_GuidedTarget(uint8_t target_type,
...
void Rover::Log_Write_Sail() {
  ...
  // @Field: VMG: Velocity made good (speed at which vehicle is making progress
  // directly towards destination)

  logger.Write(
      "SAIL", "TimeUS,Tack,TackThr,MainOut,WingOut,MastRotOut,VMG", "s-d%%%n",
      "F000000", "QBfffff", AP_HAL::micros64(), current_tack,
      (double)wind_dir_tack, (double)g2.motors.get_mainsail(),
      (double)g2.motors.get_wingsail(), (double)g2.motors.get_mast_rotation(),
...
// [AP_ShoesAgtech] ---------------------------------------------------------
// Binary log message FLWD: TimeUS, Flow (L/min EMA), FlowAvg (moving avg)
struct PACKED log_FlowData {
  LOG_PACKET_HEADER;
  uint64_t time_us;
  float flow_rate;
  float flow_rate_avg;
...
void Rover::Log_Write_Flow_Realtime(void) {
  if (!logger.logging_enabled()) {
    return;
  }

  struct log_FlowData pkt = {
    LOG_PACKET_HEADER_INIT(LOG_FLOW_DATA_MSG),
    time_us : AP_HAL::micros64(),
    flow_rate : g2.custom_nav.get_flow_rate_lmin(),
    flow_rate_avg : g2.custom_nav.get_flow_rate_avg()
  };
  logger.WriteBlock(&pkt, sizeof(pkt));
}

// [AP_ShoesAgtech] ---------------------------------------------------------
// Binary log message PHWD: pH, moving-average pH, temperature, alkalinity, ΔpH
// slot
struct PACKED log_PhData {
  ...
  float alk_mgl;
  float delta_ph;
  uint8_t slot_status; // 0=FULL 1=MORN 2=AFT 3=PREV 4=NODATA
};
...
void Rover::Log_Write_Ph_Realtime(void) {
  ...
  struct log_PhData pkt = {
    ...
    slot_status : g2.custom_nav.get_alk_slot_status()
  };
  logger.WriteBlock(&pkt, sizeof(pkt));
}
// [/AP_ShoesAgtech]
...
const LogStructure Rover::log_structure[] = {
    ...
    {LOG_THR_MSG, sizeof(log_Throttle), "THR", "Qhffff",
     "TimeUS,ThrIn,ThrOut,DesSpeed,Speed,AccX", "s--nno", "F--000"},
    ...
    // @Field: XTrack: the vehicle's current distance from the current travel
    // segment

    {LOG_NTUN_MSG, sizeof(log_Nav_Tuning), "NTUN", "QfffHf",
     "TimeUS,WpDist,WpBrg,DesYaw,Yaw,XTrack", "smhhhm", "F000B0"},
    ...
    {LOG_STEERING_MSG, sizeof(log_Steering), "STER", "Qhfffff",
     "TimeUS,SteerIn,SteerOut,DesLatAcc,LatAcc,DesTurnRate,TurnRate", "s--ookk",
     "F--0000"},
    ...
    {LOG_GUIDEDTARGET_MSG, sizeof(log_GuidedTarget), "GUIP", "QBffffff",
     "TimeUS,Type,pX,pY,pZ,vX,vY,vZ", "s-mmmnnn", "F-000000"},

    // =================================================================
    // [AP_ShoesAgtech] FLWD log structure — visible in Mission Planner Log
    // Review
    // @LoggerMessage: FLWD
    // @Description: Flow sensor measurements
    // @Field: TimeUS: Time since system startup
    // @Field: Flow: EMA filtered flow rate (L/min)
    // @Field: FlowAvg: Moving average flow rate (L/min)
    {LOG_FLOW_DATA_MSG, sizeof(log_FlowData), "FLWD", "Qff",
     "TimeUS,Flow,FlowAvg", "s--", "F--"},
    // [/AP_ShoesAgtech]

    // =================================================================
    // [AP_ShoesAgtech] PHWD log structure — pH sensor Nengshi ASPS3801D-0.5M
    // @LoggerMessage: PHWD
    // @Description: pH sensor measurements (Modbus RTU RS485)
    // @Field: TimeUS:   Time since system startup
    // @Field: pHRaw:    Raw calibrated pH reading
    // @Field: pHMA:     Moving average pH (10 samples)
    // @Field: Temp:     Water temperature (°C)
    // @Field: AlkDKH:   Estimated alkalinity (dKH)
    // @Field: AlkMGL:   Estimated alkalinity (mg/L CaCO3)
    // @Field: DeltapH:  Afternoon − morning pH swing today
    // @Field: SlotSt:   Data quality: 0=FULL 1=MORN 2=AFT 3=PREV 4=NODATA
    {LOG_PH_DATA_MSG, sizeof(log_PhData), "PHWD", "QffffffB",
     "TimeUS,pHRaw,pHMA,Temp,AlkDKH,AlkMGL,DeltapH,SlotSt", "s-------",
     "F-------"},
    // [/AP_ShoesAgtech]
};
