...
Mode::Mode()
    : ahrs(rover.ahrs), g(rover.g), g2(rover.g2),
      channel_steer(rover.channel_steer),
      channel_throttle(rover.channel_throttle),
      channel_lateral(rover.channel_lateral), channel_roll(rover.channel_roll),
...
bool Mode::enter() {
  const bool ignore_checks =
      !hal.util->get_soft_armed(); // allow switching to any mode if disarmed.
                                   // We rely on the arming check to perform
  if (!ignore_checks) {
    ...
    // check position estimate.  requires origin and at least one horizontal
    // position flag to be true
    const bool position_ok = rover.ekf_position_ok() && !rover.failsafe.ekf;
    if (requires_position() && !position_ok) {
      return false;
    }

    // check velocity estimate (if we have position estimate, we must have
    // velocity estimate)
...
void Mode::get_pilot_input(float &steering_out, float &throttle_out) const {
  ...
  default: {
    // by default regular and skid-steering vehicles reverse their rotation
    // direction when backing up
  ...
  case PilotSteerType::TWO_PADDLES: {
    ...
    // left paddle from steering input channel, right paddle from throttle input
    // channel steering = left-paddle - right-paddle throttle =
    // average(left-paddle, right-paddle)
...
void Mode::get_pilot_desired_steering_and_throttle(float &steering_out,
  ...
  if (g2.motors.have_skid_steering()) {
    const float steer_normalised =
        constrain_float(steering_out / 4500.0f, -1.0f, 1.0f);
    const float throttle_normalised =
        constrain_float(throttle_out * 0.01f, -1.0f, 1.0f);
    const float saturation_value =
        fabsf(steer_normalised) + fabsf(throttle_normalised);
  ...
  // check for special case of input and output throttle being in opposite
  // directions
  float throttle_out_limited =
      g2.motors.get_slew_limited_throttle(throttle_out, rover.G_Dt);
...
void Mode::get_pilot_desired_steering_and_speed(float &steering_out,
                                                float &speed_out) const {
  float desired_throttle;
  get_pilot_input(steering_out, desired_throttle);
  speed_out = desired_throttle * 0.01f *
              calc_speed_max(g.speed_cruise, g.throttle_cruise * 0.01f);
  // check for special case of input and output throttle being in opposite
  // directions
  float speed_out_limited = g2.attitude_control.get_desired_speed_accel_limited(
      speed_out, rover.G_Dt);
...
void Mode::get_pilot_desired_heading_and_speed(float &heading_out,
                                               float &speed_out) const {
  // get steering and throttle in the -1 to +1 range
  float desired_steering =
      constrain_float(rover.channel_steer->norm_input_dz(), -1.0f, 1.0f);
  float desired_throttle =
      constrain_float(rover.channel_throttle->norm_input_dz(), -1.0f, 1.0f);
  ...
  heading_out =
      wrap_360_cd(rad_to_cd(atan2f(desired_steering, desired_throttle)));

  // calculate throttle using magnitude of input stick vector
  const float throttle =
      MIN(safe_sqrt(sq(desired_throttle) + sq(desired_steering)), 1.0f);
  speed_out =
      throttle * calc_speed_max(g.speed_cruise, g.throttle_cruise * 0.01f);
}

// decode pilot roll and pitch inputs and return in roll_out and pitch_out
// arguments outputs are in the range -1 to +1
void Mode::get_pilot_desired_roll_and_pitch(float &roll_out,
...
void Mode::set_reversed(bool value) { g2.wp_nav.set_reversed(value); }
...
void Mode::calc_throttle(float target_speed, bool avoidance_enabled) {
  ...
  // apply object avoidance to desired speed using half vehicle's maximum
  // deceleration
    ...
    if (is_zero(target_speed) && !rover.is_balancebot()) {
      ...
      throttle_out =
          100.0f * attitude_control.get_throttle_out_stop(
                       ...
                       g2.motors.limit.throttle_upper, g.speed_cruise,
                       g.throttle_cruise * 0.01f, rover.G_Dt, stopped);
    } else {
      bool motor_lim_low =
          g2.motors.limit.throttle_lower || attitude_control.pitch_limited();
      bool motor_lim_high =
          g2.motors.limit.throttle_upper || attitude_control.pitch_limited();
      throttle_out =
          100.0f * attitude_control.get_throttle_out_speed(
                       target_speed, motor_lim_low, motor_lim_high,
                       g.speed_cruise, g.throttle_cruise * 0.01f, rover.G_Dt);
    }

    // if vehicle is balance bot, calculate actual throttle required for
    // balancing
...
bool Mode::stop_vehicle() {
  ...
  if (rover.is_balancebot()) {
    throttle_out = 100.0f * attitude_control.get_throttle_out_speed(
                                0, g2.motors.limit.throttle_lower,
                                g2.motors.limit.throttle_upper, g.speed_cruise,
                                g.throttle_cruise * 0.01f, rover.G_Dt);
    rover.balancebot_pitch_control(throttle_out);
  } else {
  ...
  if (!stopped) {
    steering_out = attitude_control.get_steering_out_rate(
        0.0, g2.motors.limit.steer_left, g2.motors.limit.steer_right,
        rover.G_Dt);
  }
...
float Mode::calc_speed_nudge(float target_speed, bool reversed) {
  ...
  float pilot_speed = pilot_throttle * 0.01f *
                      calc_speed_max(g.speed_cruise, g.throttle_cruise * 0.01f);

  // ignore pilot's input if in opposite direction to vehicle's desired
  // direction of travel note that the target_speed may be negative while
  // reversed is true (or vice-versa) while vehicle is transitioning between
...
void Mode::navigate_to_waypoint() {
  ...
  // calc_speed_nudge's "desired_speed" argument should be negative when vehicle
  // is reversing AR_WPNav nudge_speed_max argument should always be positive
  // even when reversing
  const float calc_nudge_input_speed =
      g2.wp_nav.get_speed_max() * (g2.wp_nav.get_reversed() ? -1.0f : 1.0f);
  ...
  // Duy - Update safety pitch (Execute Pitch Safety Rate Limiting)
  const uint32_t now_ms = AP_HAL::millis();
  const float wp_speed_instant = g2.wp_nav.get_speed();
  const float wp_speed_max = g2.wp_nav.get_speed_max();

  float desired_speed = wp_speed_instant;
  ...
  // 1. Kiểm tra dải giá trị đầu vào của tham số cấu hình (Hỗ trợ từ 0% đến
  // 100%)
  const float pitch_scale_param = g.bst_pitch_scale.get() * 0.01f;

  // Cho phép hạ dải chặn xuống hoàn toàn 0.0f (0%) thay vì giữ mức sàn 0.1f
  const float pitch_scale = constrain_float(pitch_scale_param, 0.0f, 1.0f);

  // 2. Thực thi kiểm soát tốc độ đích (Target Speed Management)
  if (is_pitch_bad) {
    ...
    _pitch_safe_start_ms = 0U; // Clear recovery timer

    if (!_pitch_warning_sent) {
      gcs().send_text(MAV_SEVERITY_CRITICAL,
                      "[AUTO] PITCH DANGER: %.2f deg (LIMIT SPEED TO %.2f m/s)",
    ...
    const uint32_t recovery_delay_ms =
        static_cast<uint32_t>(MAX(g.bst_pitch_delay.get(), 0));
    if (now_ms - _pitch_safe_start_ms >= recovery_delay_ms) {
      gcs().send_text(MAV_SEVERITY_INFO,
                      "[AUTO] Pitch Safe - Resuming normal speed");
      _pitch_warning_sent = false;
      _pitch_safe_start_ms = 0U;
    } else {
      // Maintain scaled speed limit during recovery delay periods
      const float max_allowed_speed = wp_speed_max * pitch_scale;
      if (desired_speed > max_allowed_speed) {
        desired_speed = max_allowed_speed;
      }
  ...
  // Pass desired speed to throttle and attitude controller
  ...
  if (g2.sailboat.use_indirect_route(desired_heading_cd)) {
    ...
    if (g2.avoid.limits_active() &&
        (fabsf(g2.attitude_control.get_desired_speed()) <=
         g2.attitude_control.get_stop_speed())) {
...
void Mode::calc_steering_from_turn_rate(float turn_rate) {
  ...
  const float steering_out = attitude_control.get_steering_out_rate(
      turn_rate, g2.motors.limit.steer_left, g2.motors.limit.steer_right,
      rover.G_Dt);
...
*/
void Mode::calc_steering_from_lateral_acceleration(float lat_accel,
                                                   bool reversed) {
  // constrain to max G force
  lat_accel =
      constrain_float(lat_accel, -attitude_control.get_turn_lat_accel_max(),
                      attitude_control.get_turn_lat_accel_max());

  // send final steering command to motor library
  const float steering_out = attitude_control.get_steering_out_lat_accel(
      lat_accel, g2.motors.limit.steer_left, g2.motors.limit.steer_right,
      rover.G_Dt);
  set_steering(steering_out * 4500.0f);
}
...
void Mode::calc_steering_to_heading(float desired_heading_cd,
  ...
  const float steering_out = attitude_control.get_steering_out_heading(
      radians(desired_heading_cd * 0.01f), radians(rate_max_degs),
      g2.motors.limit.steer_left, g2.motors.limit.steer_right, rover.G_Dt);
  set_steering(steering_out * 4500.0f);
}
