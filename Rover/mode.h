...
public:
  ...
  //
  // methods that sub classes should override to affect movement of the vehicle
  // in this mode
  //
  ...
  // return if external control is allowed in this mode (Guided or
  // Guided-within-Auto)
  virtual bool in_guided_mode() const { return false; }

  // returns true if vehicle can be armed or disarmed from the transmitter in
  // this mode
  ...
  // return heading (in degrees) and cross track error (in meters) for reporting
  // to ground station (NAV_CONTROLLER_OUTPUT message)
  ...
  // set next_destination (if known).  If not provided vehicle stops at
  // destination
  virtual bool
  set_desired_location(const Location &destination,
                       Location next_destination = Location()) WARN_IF_UNUSED;

  // true if vehicle has reached desired location. defaults to true because this
  // is normally used by missions and we do not want the mission to become stuck
  virtual bool reached_destination() const { return true; }

  // get default speed for this mode (held in CRUISE_SPEED, WP_SPEED or
  // RTL_SPEED) rtl argument should be true if called from RTL or SmartRTL modes
  // (handled here to avoid duplication)
...
protected:
  ...
  // subclasses override this to perform any required cleanup when exiting the
  // mode
  virtual void _exit() { return; }

  // decode pilot steering and throttle inputs and return in steer_out and
  // throttle_out arguments steering_out is in the range -4500 ~ +4500 with
  // positive numbers meaning rotate clockwise throttle_out is in the range -100
  // ~ +100
  void get_pilot_desired_steering_and_throttle(float &steering_out,
                                               float &throttle_out) const;

  // decode pilot input steering and return steering_out and speed_out (in m/s)
  void get_pilot_desired_steering_and_speed(float &steering_out,
  ...
  void get_pilot_desired_heading_and_speed(float &heading_out,
                                           float &speed_out) const;

  // decode pilot roll and pitch inputs and return in roll_out and pitch_out
  // arguments outputs are in the range -1 to +1
  void get_pilot_desired_roll_and_pitch(float &roll_out,
                                        float &pitch_out) const;
  ...
  // rate_max is a maximum turn rate in deg/s.  set to zero to use default turn
  // rate limits
  void calc_steering_to_heading(float desired_heading_cd,
  ...
  //  reversed should be true if the vehicle is intentionally backing up which
  //  allows the pilot to increase the backing up speed by pulling the throttle
  //  stick down
...
protected:
  ...
  float _distance_to_destination; // distance from vehicle to final destination
                                  // in meters
  bool
      _reached_destination; // true once the vehicle has reached the destination
...
public:
  ...
  bool
  get_desired_location(Location &destination) const override WARN_IF_UNUSED;
  bool set_desired_location(const Location &destination,
                            Location next_destination = Location()) override
      WARN_IF_UNUSED;
  ...
  AP_Mission mission{FUNCTOR_BIND_MEMBER(&ModeAuto::start_command, bool,
                                         const AP_Mission::Mission_Command &),
                     FUNCTOR_BIND_MEMBER(&ModeAuto::verify_command_callback,
                                         bool,
                                         const AP_Mission::Mission_Command &),
...
protected:
  ...
  enum SubMode : uint8_t {
    ...
    Guided, // handover control to external navigation system from within auto
            // mode
    Stop,   // stop the vehicle as quickly as possible
    NavScriptTime, // accept targets from lua scripts while NAV_SCRIPT_TIME
...
private:
  ...
  bool waiting_to_start; // true if waiting for EKF origin before starting
                         // mission
  ...
  bool _reached_heading; // true when vehicle has reached desired heading in
                         // TurnToHeading sub mode

  // Loiter control
  uint16_t loiter_duration;   // How long we should loiter at the nav_waypoint
                              // (time in seconds)
  uint32_t loiter_start_time; // How long have we been loitering - The start
                              // time in millis
  bool
      previously_reached_wp; // set to true if we have EVER reached the waypoint

  // Guided-within-Auto variables
  struct {
    Location loc;          // location target sent to external navigation
    bool valid;            // true if loc is valid
    uint32_t last_sent_ms; // system time that target was last sent to offboard
                           // navigation
  } guided_target;
  ...
  // For example in a delay command the condition_start records that start time
  // for the delay
  ...
  struct {
    ...
    uint16_t id; // unique id to avoid race conditions between commands and lua
                 // scripts
    uint32_t start_ms; // system time nav_script_time command was received (used
                       // for timeout)
  ...
  // Shoes_Agtech: AUTO_TUNE - thu thap chi so on dinh 1 chu ky Auto
  // (arm->disarm hoac doi mode) roi in recommend PID, khong tu ghi de tham so
  void _autotune_poll();
  void _autotune_start();
  void _autotune_finish();
  void _autotune_sample_speed(float target_speed);
...
public:
  ...
  // return the distance at which the vehicle is considered to be on track along
  // the circle
  ...
  // return desired heading (in degrees) and cross track error (in meters) for
  // reporting to ground station (NAV_CONTROLLER_OUTPUT message)
  float wp_bearing() const override;
  float nav_bearing() const override;
...
protected:
  ...
  // if there is no current position estimate target_yaw_rad is set to vehicle
  // yaw
  ...
  struct {
    ...
    Vector2f center_pos; // circle center as an offset (in meters) from the EKF
                         // origin
  ...
  float dist_to_edge_m;  // distance to edge of circle in meters (equivalent to
                         // crosstrack error)
  bool tracking_back;    // true if the vehicle is trying to track back onto the
                         // circle
};
...
public:
  ...
  void set_desired_heading_delta_and_speed(float yaw_delta_cd,
                                           float target_speed);
...
protected:
  ...
  enum class Options : int32_t { SCurvesUsedForNavigation = (1U << 6) };

  bool _enter() override;

  // returns true if GUID_OPTIONS bit set to use scurve navigation instead of
  // position controller input shaping scurves provide path planning and object
  // avoidance but cannot handle fast updates to the destination (for fast
  // updates use position controller input shaping)
  ...
  uint32_t _des_att_time_ms;   // system time last call to set_desired_attitude
                               // was made (used for timeout)
  float _desired_yaw_rate_cds; // target turn rate centi-degrees per second
  bool
      send_notification; // used to send one time notification to ground station
  float _desired_speed;  // desired speed used only in HeadingAndSpeed submode

  // direct steering and throttle control
  bool _have_strthr; // true if we have a valid direct steering and throttle
                     // inputs
  uint32_t
      _strthr_time_ms; // system time last call to set_steering_and_throttle was
                       // made (used for timeout)
  ...
  struct {
    ...
    float horiz_max; // horizontal position limit in meters from where guided
                     // mode was initiated (0 = no limit)
    uint32_t start_time_ms; // system time in milliseconds that control was
...
protected:
  ...
  float
      _desired_speed; // desired speed (ramped down from initial speed to zero)
};
...
private:
  float _desired_lat_accel; // desired lateral acceleration calculated from
...
private:
  float _initial_heading_cd; // vehicle heading (in centi-degrees) at moment
                             // vehicle was armed
  float _desired_heading_cd; // latest desired heading (in centi-degrees) from
                             // pilot
};
...
protected:
  ...
  AP_Float stopping_dist;   // how far away from the docking target should we
                            // start stopping

  bool _enter() override;

  // return reduced speed of vehicle based on error in position and current
  ...
  // we force the vehicle to use real dock target vector when this much close to
  // the docking station
  const float _force_real_target_limit_cm = 300.0f;
  // acceptable lateral error in vehicle's position with respect to dock. This
  // is used while slowing down the vehicle
  const float _acceptable_pos_error_cm = 20.0f;

  Vector2f _dock_pos_rel_origin_cm; // position vector towards docking target
                                    // relative to ekf origin
  Vector2f _desired_heading_NE;   // unit vector in desired direction of docking
  bool _docking_complete = false; // flag to mark docking complete when we are
                                  // close enough to the dock
